...
class Jogador:
    ...
    def pagar_contas(self):
        pop_up("Hora de pagar suas dividas")
        if self.renda >= self.dividas:
            self.renda -= self.dividas
        else:
            self.dividas += 200
...
def desafio_financeiro(jogador):
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            pop_up("""Voce recebeu uma erança de seu pai
                   A) Preciso desse dinheiro para me ajudar B) Não tem porque eu usar isso, vou guardar""")
            if event.key == pygame.K_RIGHT:
                break
            if event.kay == pygame.K_LEFTH:
                jogador.faturamento += 1000
                jogador.mover()
                jogador.mover()
                break
